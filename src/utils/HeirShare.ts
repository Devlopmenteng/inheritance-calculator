...
export class HeirShare {
  ...
  constructor(options: {
    ...
    this.name =
      options.name || FIQH_DATABASE.heirNames[options.key] || options.key;
    this.type = options.type || 'فرض';
    this.count = options.count || 1;
    this.fraction =
      options.fraction instanceof Fraction ? options.fraction : new Fraction(0);
  ...
  setFraction(fraction: Fraction | number): void {
    this.fraction =
      fraction instanceof Fraction ? fraction : new Fraction(fraction);
  }
