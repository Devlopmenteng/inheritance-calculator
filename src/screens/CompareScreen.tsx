...
const CompareScreen: React.FC = () => {
  ...
  const [results, setResults] = useState<
    Record<MadhabType, ReturnType<InheritanceEngine['calculate']> | null>
  >({
    shafii: null,
    hanafi: null,
    maliki: null,
  ...
  const runComparison = async () => {
    ...
    const newResults: Record<
      MadhabType,
      ReturnType<InheritanceEngine['calculate']> | null
    > = {
      shafii: null,
      hanafi: null,
      maliki: null,
        ...
        <Card style={styles.card} mode="elevated">
          <Card.Content>
            <Text style={styles.infoText}>
              سيتم مقارنة حساب الميراث للبيانات المدخلة في الحاسبة الرئيسية عبر
              المذاهب الأربعة.
            </Text>
          ...
          <Card style={styles.card} mode="elevated">
            ...
            <Card.Content>
              <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                <DataTable>
                  <DataTable.Header>
                    <DataTable.Title style={styles.heirColumn}>
                      الوارث
                    </DataTable.Title>
                    {madhabs.map((madhab) => (
                      <DataTable.Title
                        ...
                        return (
                          <DataTable.Cell
                            ...
                            style={[
                              styles.madhabColumn,
                              {
                                backgroundColor: `${getMadhabColor(madhab)}08`,
                              },
                            ]}
                          >
                            {share ? (
                      ...
                      <View style={styles.chipsRow}>
                        ...
                        {result.raddApplied && (
                          <Chip
                            ...
                            textStyle={{ fontSize: 10, color: '#1565C0' }}
                          >
                            رادّة
                          </Chip>
                        )}
                        {result.bloodRelativesApplied && (
                          <Chip
                            ...
                            textStyle={{ fontSize: 10, color: '#7B1FA2' }}
                          >
                            ذوو أرحام
                          </Chip>
                        )}
              ...
              <View style={styles.notesContainer}>
                ...
                {madhabs.map((madhab) => {
                  const result = results[madhab];
                  if (!result?.success || !result.madhhabNotes?.length)
                    return null;
