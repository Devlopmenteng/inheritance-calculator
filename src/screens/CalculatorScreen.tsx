...
import { InheritanceEngine, type Heirs } from '../utils/InheritanceEngine';
    ...
    heirs: [
      {
        ...
        label: 'الأب',
        max: 1,
        description: '⅙ + تعصيب أو تعصيب فقط',
      },
      {
        key: 'mother',
        label: 'الأم',
        max: 1,
        description: '⅙ أو ⅓ أو ثلث الباقي',
      },
      {
        key: 'grandfather',
        label: 'الجد الصحيح',
        max: 1,
        description: 'أبو الأب وإن علا',
      },
      {
        key: 'grandmother_mother',
        label: 'الجدة لأم',
        max: 1,
        description: '⅙ عند عدم الأم',
      },
      {
        key: 'grandmother_father',
        label: 'الجدة لأب',
        max: 1,
        description: '⅙ عند عدم الأم والأب',
      },
    ],
  },
  {
    ...
    heirs: [
      { key: 'son', label: 'الابن', max: 50, description: 'عصبة بالنفس' },
      {
        ...
        label: 'البنت',
        max: 50,
        description: '½ أو ⅔ أو عصبة بالغير',
      },
      {
        key: 'grandson',
        label: 'ابن الابن',
        max: 50,
        description: 'عصبة وإن نزل',
      },
      {
        key: 'granddaughter',
        label: 'بنت الابن',
        max: 50,
        description: '⅙ تكملة أو ⅔',
      },
    ],
  },
  {
    ...
    heirs: [
      {
        ...
        label: 'الأخ الشقيق',
        max: 50,
        description: 'عصبة بالنفس',
      },
      {
        key: 'full_sister',
        label: 'الأخت الشقيقة',
        max: 50,
        description: '½ أو ⅔ أو عصبة',
      },
      {
      ...
      },
      {
        ...
        label: 'الأخت لأب',
        max: 50,
        description: '⅙ تكملة أو عصبة',
      },
      {
        key: 'maternal_brother',
        label: 'الأخ لأم',
        max: 50,
        description: '⅙ أو ⅓ بالتساوي',
      },
      {
        key: 'maternal_sister',
        label: 'الأخت لأم',
  ...
  },
  {
    ...
    heirs: [
      ...
      },
      {
        ...
        label: 'أولاد الأخت',
        max: 50,
        description: 'ذو رحم - صنف 2',
      },
      {
        key: 'maternal_uncle',
        label: 'الخال',
        max: 50,
        description: 'ذو رحم - صنف 3',
      },
      {
        key: 'maternal_aunt',
        label: 'الخالة',
...
const CalculatorScreen: React.FC = () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        addAuditLog(
          'حساب الميراث',
          'error',
          result.errors?.join(', ') || 'خطأ غير معروف',
        );
      }
    } catch (error: any) {
  ...
  const handleReset = useCallback(() => {
    Alert.alert('إعادة تعيين', 'هل أنت متأكد من إعادة تعيين جميع البيانات؟', [
  ...
  const loadQuickTest = useCallback(
    (testHeirs: Record<string, number>) => {
      resetHeirs();
  ...
  const netEstate = Math.max(
    0,
    estate.total - estate.funeral - estate.debts - estate.will,
  );
      ...
      >
        ...
        <View style={styles.statsContainer}>
          ...
          <Surface
            style={[
              styles.statCard,
              { borderColor: madhabConfig.color, borderWidth: 2 },
            ]}
            elevation={2}
          >
            <Text style={[styles.statValue, { color: madhabConfig.color }]}>
              {madhabConfig.name}
            </Text>
          ...
          <Card.Title
            ...
            left={(props) => (
              <Avatar.Icon
                ...
                icon="mosque"
                size={40}
                style={{ backgroundColor: madhabConfig.color }}
              />
            )}
          />
          <Card.Content>
            <View style={styles.madhabContainer}>
              {(Object.keys(FIQH_DATABASE.madhabs) as MadhabType[]).map(
                (madhab) => {
            ...
            <Surface
              style={[
                styles.madhabInfo,
                { backgroundColor: `${madhabConfig.color}15` },
              ]}
            >
              <Text
                style={[styles.madhabInfoTitle, { color: madhabConfig.color }]}
              >
                {madhabConfig.icon} المذهب {madhabConfig.name}
              </Text>
              <Text style={styles.madhabInfoDesc}>
        ...
        <Card style={styles.card} mode="elevated">
          ...
          <Card.Content>
            <TextInput
              ...
              onChangeText={(text) =>
                updateEstateField('total', parseFloat(text) || 0)
              }
            ...
            <View style={styles.rowInputs}>
              <TextInput
                ...
                onChangeText={(text) =>
                  updateEstateField('funeral', parseFloat(text) || 0)
                }
                keyboardType="numeric"
                mode="outlined"
              ...
              <TextInput
                ...
                onChangeText={(text) =>
                  updateEstateField('debts', parseFloat(text) || 0)
                }
                keyboardType="numeric"
                mode="outlined"
                style={[styles.input, styles.flex1]}
            ...
            <TextInput
              ...
              onChangeText={(text) =>
                updateEstateField('will', parseFloat(text) || 0)
              }
              keyboardType="numeric"
              mode="outlined"
              style={styles.input}
            ...
            <Surface style={styles.notice}>
              <Text style={styles.noticeText}>
                ⚠️ ترتيب الحقوق الشرعي: ١. تكاليف التجهيز ← ٢. سداد الديون ← ٣.
                الوصية (≤ ⅓) ← ٤. الإرث
              </Text>
            </Surface>
          </Card.Content>
